...
__all__ = [
    'ProphetNetModel', 'ProphetNetPretrainedModel', 'ProphetNetEncoder', 'ProphetNetDecoder',
...
def compute_relative_buckets(num_buckets, max_distance, relative_positions, is_bidirectional=False):
    ...
    if is_bidirectional:
        ...
        rel_positions_bucket = (rel_positions_bucket + paddle.cast(paddle.less_than(inv_relative_positions,
                                                                                    paddle.zeros_like(
                                                                                        inv_relative_positions)),
                                                                   dtype=paddle.int32) * num_buckets)
        inv_relative_positions = paddle.abs(inv_relative_positions)
    else:
        inv_relative_positions = paddle.cast(paddle.less_than(paddle.zeros_like(inv_relative_positions),
                                                              inv_relative_positions),
    ...
    val_if_large = max_exact + paddle.log(paddle.cast(inv_relative_positions, dtype=paddle.float32)
                                          / max_exact) / math.log(max_distance / max_exact) * (num_buckets - max_exact)
    ...
    rel_positions_bucket = rel_positions_bucket + paddle.where(is_small, paddle.cast(inv_relative_positions,
                                                                                     dtype=paddle.int32),
                                                               val_if_large)
...
def compute_all_stream_relative_buckets(num_buckets, max_distance, position_ids):
    ...
    """
    # main stream
    main_stream_relative_positions = paddle.tile(paddle.unsqueeze(position_ids, axis=1),
    ...
    # predicting stream
    predicting_stream_relative_positions = paddle.unsqueeze(paddle.concat([position_ids - 1, position_ids],
                                                                          axis=-1), axis=1)
    predicting_stream_relative_positions = paddle.tile(predicting_stream_relative_positions,
                                                       repeat_times=[1, position_ids.shape[-1], 1])
    predicting_stream_relative_positions = predicting_stream_relative_positions - paddle.unsqueeze(position_ids,
                                                                                                   axis=-1)
...
class ProphetNetPretrainedModel(PretrainedModel):
    ...
    def init_weights(self, layer):
        if isinstance(layer, nn.Linear):
            layer.weight.set_value(paddle.tensor.normal(mean=0.0,
    ...
    def _shift_right(self, input_ids):
        ...
        assert paddle.sum(paddle.cast(shifted_input_ids >= 0, dtype=paddle.int32)).item() == shifted_input_ids.shape[
    ...
    def forward(self, inputs_shape, attention_mask=None, past_key_values=None, position_ids=None):
        ...
        if position_ids is None:
            if past_key_values is not None:
                ...
                position_ids = paddle.ones((1, 1), dtype='int64') * (
                    int(self._padding_idx + num_input_ids)
                )
            else:
                ...
                position_ids = paddle.cast(
                    ...
                    attention_mask, dtype=paddle.int64) + self._padding_idx
...
class ProphetNetAttention(Layer):
    ...
    def _shape(self, tensor: paddle.Tensor, seq_len: int, bsz: int):
        return paddle.transpose(paddle.reshape(tensor, [bsz, seq_len, self.num_attn_heads, self.head_dim]),
                                (0, 2, 1, 3))
    ...
    def forward(self,
                ...
                past_key_value: Optional[Tuple[Tensor]] = None) -> Tuple[Tensor, Optional[Tensor]]:
        ...
        attn_output = (paddle.reshape(paddle.transpose(paddle.reshape(attn_output,
                                                                      (batch_size, self.num_attn_heads, tgt_len,
                                                                       self.head_dim)),
                                                       (0, 2, 1, 3)),
...
class ProphetNetNgramSelfAttention(Layer):
    ...
    def _shape(self, tensor, seq_len, batch_size):
        return paddle.transpose(paddle.reshape(tensor,
    ...
    def forward(self,
        ...
        # Update cache
        past_key_value = (paddle.reshape(main_key_states, (batch_size, self.num_attn_heads, -1, self.head_dim)),
        ...
        main_relative_pos_embeddings = self.get_main_relative_pos_embeddings(main_hidden_states,
                                                                             main_attn_weights,
                                                                             position_ids,
        ...
        main_attn_output = (paddle.reshape(paddle.transpose(
            paddle.reshape(main_attn_output, (batch_size, self.num_attn_heads, sequence_length, self.head_dim))
            , (0, 2, 1, 3)), (batch_size, 1, sequence_length, hidden_size)))
        main_attn_output = self.out_proj(main_attn_output)

        # PREDICT-STREAM
        # [ngram, B*head, T, c]
        predict_query_states = paddle.reshape(paddle.concat(predict_query_states_list, axis=0),
                                              (self.ngram, -1, sequence_length, self.head_dim))
        # [ngram, B*head, 2*T, c]
        predict_key_states = paddle.concat([paddle.unsqueeze(paddle.concat([main_key_states, key], axis=1), axis=0)
                                            for key in predict_key_states_list], axis=0)

        # [ngram, T, B, C]
        predict_hidden_states = paddle.reshape(paddle.concat(hidden_states_predict_list, axis=0),
                                               (self.ngram, sequence_length, batch_size, hidden_size))

        # [ngram, B*head, 2*T, c]
        predict_value_states = paddle.concat([paddle.unsqueeze(paddle.concat([main_value_states, v_p], axis=1), axis=0)
                                              for v_p in predict_value_states_list], axis=0)

        # [ngram, B*head, T, 2*T]
        predict_attn_weights = paddlenlp.ops.einsum("nbtc,nbsc->nbts", (predict_query_states, predict_key_states))

        # [ngram, B*head, T, S]
        # retrieve relative position embeddings for each layer -> see paper for more details
        predict_relative_pos_embeddings = self.get_predict_relative_pos_embeddings(predict_hidden_states,
                                                                                   predict_attn_weights, position_ids,
        ...
        if extended_predict_attention_mask is not None:
            predict_attn_weights = predict_attn_weights + paddle.cast(extended_predict_attention_mask,
        ...
        # [ngram, B*head, T, c]
        predict_attn_output = paddlenlp.ops.einsum("nbts,nbsc->nbtc", (predict_attn_probs, predict_value_states))

        # reshape so that num_heads dim is merged into last `head_dim` axis
        # [ngram, B, T, C]
        predict_attn_output = (paddle.reshape(paddle.transpose(paddle.reshape(predict_attn_output, (self.ngram,
                                                                                                    batch_size,
                                                                                                    self.num_attn_heads,
                                                                                                    sequence_length,
                                                                                                    self.head_dim)),
        ...
        # [B, 1+ngram*T, C]
        attn_output = paddle.reshape(paddle.concat([main_attn_output, predict_attn_output], axis=1),
        ...
        predict_attn_probs = paddle.transpose(paddle.reshape(predict_attn_probs, (self.ngram,
                                                                                  batch_size,
                                                                                  self.num_attn_heads,
                                                                                  sequence_length, -1)),
                                              (1, 0, 2, 3, 4))
    ...
    def get_main_relative_pos_embeddings(self, hidden_states, attn_weights, position_ids,
                                         main_relative_position_buckets):
        # input hidden_states [B,T,C], input attn_weights [T*head,T,S], input position_ids [B,T] or [1,1]

        if main_relative_position_buckets is None:
            batch_size, sequence_length = hidden_states.shape[:2]
            relative_positions = (paddle.tile(paddle.unsqueeze(paddle.unsqueeze(paddle.arange(1,
                                                                                              attn_weights.
                                                                                              shape[-1] + 1),
                                                                                axis=0),
                                                               axis=0),
                                              repeat_times=[batch_size, sequence_length, 1]))
            relative_positions = relative_positions - paddle.tile(paddle.unsqueeze(position_ids, axis=0),
                                                                  repeat_times=[batch_size, sequence_length,
                                                                                1])  # [B, T, s]
            main_relative_position_buckets = compute_relative_buckets(
        ...
        rel_pos_embeddings = paddle.transpose(paddle.reshape(rel_pos_embeddings,
        ...
        main_relative_position_buckets = paddle.cast(paddle.reshape(paddle.tile(main_relative_position_buckets,
                                                                                repeat_times=[1, self.num_attn_heads,
                                                                                              1]),
        ...
        main_relative_position_buckets_index = paddle.tile(main_relative_position_buckets.unsqueeze(2),
                                                           repeat_times=[1, 1, 2])
        ...
        main_relative_pos_embeddings = paddle.reshape(paddle.gather_nd(rel_pos_embeddings,
                                                                       index=main_relative_position_buckets_index),
                                                      (attn_weights.shape[:2] + [-1]))
        return main_relative_pos_embeddings

    def get_predict_relative_pos_embeddings(self, hidden_states, attn_weights, position_ids,
                                            predict_relative_position_buckets):
        ...
        if predict_relative_position_buckets is None:
            ...
            relative_positions = (paddle.tile(paddle.unsqueeze(paddle.unsqueeze(paddle.arange(0, key_sequence_length),
                                                                                axis=0),
                                                               axis=0),
                                              repeat_times=[batch_size, sequence_length, 1]))
            ...
            relative_positions = relative_positions - paddle.tile(paddle.unsqueeze(position_ids, axis=0),
                                                                  repeat_times=[batch_size, sequence_length, 1])
            predict_relative_position_buckets = compute_relative_buckets(self.num_buckets,
                                                                         self.relative_max_distance,
                                                                         relative_positions, False)

        hidden_states = paddle.transpose(hidden_states, (0, 2, 1, 3))  # [ngram, B, T, C]
        rel_pos_embeddings = paddle.reshape(self.relative_pos_embeddings(hidden_states),
                                            hidden_states.shape[:-1] +
                                            [self.num_buckets,
                                             self.num_attn_heads])  # [ngram, B, T, bucket, head]
        rel_pos_embeddings = paddle.reshape(paddle.transpose(rel_pos_embeddings,
                                                             (0, 1, 4, 2, 3)),
                                            (self.ngram * batch_size * self.num_attn_heads,
                                             sequence_length, -1))  # [ngram*B*head, T, bucket]

        predict_relative_position_buckets = paddle.tile(paddle.unsqueeze(predict_relative_position_buckets,
                                                                         axis=0),
        ...
        predict_relative_position_buckets = paddle.cast(paddle.reshape(predict_relative_position_buckets,
                                                                       (-1,
                                                                        predict_relative_position_buckets.shape[-1])),
                                                        dtype=paddle.int64)  # [ngram*B*head*T, S]

        predict_relative_position_buckets_index = paddle.tile(predict_relative_position_buckets.unsqueeze(2),
                                                              repeat_times=[1, 1, 2])
        ...
        predict_relative_pos_embeddings = paddle.reshape(paddle.gather_nd(rel_pos_embeddings,
                                                                          index=predict_relative_position_buckets_index),
                                                         (self.ngram, batch_size * self.num_attn_heads,
                                                          sequence_length, -1))  # [ngram, B*head, T, S]
...
class ProphetNetEncoderLayer(Layer):
    ...
    def __init__(self,
        ...
        # 2nd residual block
        self.feed_forward = ProphetNetFeedForward(hidden_size, activation_function, activation_dropout, dropout,
                                                  encoder_ffn_dim)
...
class ProphetNetDecoderLayer(Layer):
    ...
    def __init__(self,
        ...
        # 1st residual block
        self.self_attn = ProphetNetNgramSelfAttention(hidden_size, num_buckets, relative_max_distance,
...
class ProphetNetEncoder(ProphetNetPretrainedModel):
    ...
    def __init__(self,
        ...
        self.layers = nn.LayerList([ProphetNetEncoderLayer(hidden_size,
                                                           ...
                                                           attention_dropout,
                                                           dropout,
                                                           num_encoder_attention_heads) for _ in
    ...
    def forward(self,
        ...
        if attention_mask is not None:
            extended_attention_mask = (paddle.tile(1.0 - attention_mask.unsqueeze(1),
                                                   repeat_times=[self.config["num_encoder_attention_heads"],
                                                                 1, 1])) * -10000.0
...
class ProphetNetDecoder(ProphetNetPretrainedModel):
    def __init__(self,
        ...
        self.layers = nn.LayerList([ProphetNetDecoderLayer(hidden_size,
                                                           num_buckets,
                                                           relative_max_distance,
                                                           num_decoder_attention_heads,
                                                           activation_function,
    ...
    def forward(self,
        ...
        main_stream_pos_embed, position_ids = self.position_embeddings((batch_size, sequence_length),
        ...
        if past_key_values is not None:
            ...
            ngram_hidden_states = [paddle.tile((ngram_embeddings[ngram - 1] + predicting_stream_pos_embed),
        ...
        if encoder_attention_mask is not None:
            extended_encoder_attention_mask = (1.0 - paddle.tile(encoder_attention_mask[:, None, :],
                                                                 repeat_times=[
        ...
        return tuple(v for v in [last_hidden_state,
                                 last_hidden_state_ngram,
                                 present_key_values] if v is not None)
    ...
    def compute_buffered_relative_buckets(self, position_ids):
        ...
        main_relative_buckets = paddle.tile(self._main_relative_buckets[:, :sequence_length, :sequence_length],
                                            repeat_times=[batch_size, 1, 1])
        predict_relative_buckets = paddle.tile(paddle.concat(
            [self._predict_relative_buckets[:, :sequence_length, :sequence_length],
             ...
             self.max_target_positions: self.max_target_positions + sequence_length]], axis=2),
            repeat_times=[batch_size, 1, 1])
    ...
    def prepare_attention_mask(self, hidden_states, attention_mask):
        ...
        if not hasattr(self, '_causal_mask') or self._causal_mask is None:
            causal_mask = paddle.full((self.max_target_positions, self.max_target_positions), -float("inf"),
                                      dtype=hidden_states.dtype)
            self._causal_mask = paddle.triu(causal_mask, 1)
        extended_causal_mask = paddle.expand(self._causal_mask[:seq_length, :seq_length].unsqueeze(0),
        ...
        return paddle.cast(paddle.tile(extended_attention_mask,
    ...
    def prepare_predict_attention_mask(self, hidden_states, attention_mask):
        ...
        if not hasattr(self, '_predict_causal_mask') or self._predict_causal_mask is None:
            self._predict_causal_mask = ngram_attention_bias(self.max_target_positions, self.ngram, hidden_states.dtype)
        predict_causal_mask = paddle.concat([self._predict_causal_mask[:, :seq_length, :seq_length],
                                             self._predict_causal_mask[:, :seq_length,
                                             self.max_target_positions: self.max_target_positions + seq_length]],
                                            axis=-1)
        extended_predict_causal_mask = paddle.expand(predict_causal_mask[:, None, :, :],
                                                     shape=predict_causal_mask.shape[:1] + [
                                                         batch_size] + predict_causal_mask.shape[1:])
        ...
        if attention_mask is not None:
            ...
            extended_attention_mask = paddle.concat([extended_attention_mask,
                                                     paddle.zeros_like(extended_attention_mask)],
                                                    axis=-1)
            extended_predict_attention_mask = extended_predict_causal_mask + extended_attention_mask
        else:
            extended_predict_attention_mask = extended_predict_causal_mask
        return paddle.cast(extended_predict_attention_mask.tile([1, self.config["num_decoder_attention_heads"], 1, 1]),
                           dtype=hidden_states.dtype)
...
class ProphetNetModel(ProphetNetPretrainedModel):
    def __init__(self,
        ...
        self.encoder = ProphetNetEncoder(self.word_embeddings,
                                         vocab_size,
                                         hidden_size,
                                         pad_token_id,
                                         max_position_embeddings,
        ...
        self.decoder = ProphetNetDecoder(self.word_embeddings,
                                         vocab_size,
                                         hidden_size,
                                         pad_token_id,
                                         max_position_embeddings,
...
class Linear_wo_bias(Layer):
    ...
    def forward(self, input):
        out = F.linear(
            x=input, weight=self.weight, name=self.name)
        return out
...
class ProphetNetForConditionalGeneration(ProphetNetPretrainedModel):
    ...
    def forward(self,
        ...
        predicting_streams = paddle.reshape(outputs[1],
